...
// Sample product data generators
const productNames = [
...
function generateImages(productName: string): string[] {
  const baseImages = [
...
function generateFeatures(productName: string, category: string): string[] {
  ...
  return features.slice(0, Math.floor(Math.random() * 3) + 3); // 3-5 features
}

function generateProductData(shopId: string, index: number): Partial<Product> {
  const productName = productNames[Math.floor(Math.random() * productNames.length)];
  const category = productCategories[Math.floor(Math.random() * productCategories.length)];
  const basePrice = generateRandomPrice(15, 300);
  const isSubscription = Math.random() > 0.7; // 30% chance of subscription
  const isPhysical = Math.random() > 0.2; // 80% chance of physical product

  const productData: Partial<Product> = {
    name: `${productName} - Shop ${index}`,
  ...
  if (isPhysical) {
    productData.weight = Number((Math.random() * 2 + 0.1).toFixed(2)); // 0.1-2.1 kg
...
async function seedProducts() {
  ...
  try {
    ...
    const shopsResponse = await shopsService.findAll({ limit: 100 }); // Get up to 100 shops
    ...
    for (const shop of shops) {
      ...
      const shopWithProducts = await shopsService.findById(shop.id);
      const existingProductCount = shopWithProducts.products ? shopWithProducts.products.length : 0;
      ...
      for (let i = existingProductCount + 1; i <= 10; i++) {
        ...
        try {
          ...
          console.log(`   ✅ Created product ${i}: ${product.name} - $${product.basePrice}`);
        } catch (error) {
